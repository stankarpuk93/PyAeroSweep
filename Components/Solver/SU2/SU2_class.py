...
class SU2():
            ...
            for j in range(len_Mach):
                for k in range(len_AoA): 
                        ...
                        if (k == 0 and warstart_set== 'YES') or warstart_set == 'NO':
                            ...
                            self.warmstart = 'YES'
                        filename = SU2.run_SU2_config(self,Freestream.Altitude[i],Freestream.Mach[j],Freestream.Angle_of_attack,\
                        ...
                        self.run_SU2(self.processors,filename)
                        print('Solution ' + filename + ' Completed')

                        # Read results
                        Cl[i,j,k],Cd[i,j,k],Cm[i,j,k] = SU2.read_results('SU2_output.log')
    ...
    def run_SU2(self,processors,filename):
        ...
        with open('SU2_output.log', 'w') as f:
            ...
            #subprocess.Popen(['mpiexec', '-n',str(processors),'SU2_CFD', filename], stdout= f, stderr= sys.stderr, stdin=subprocess.PIPE)
