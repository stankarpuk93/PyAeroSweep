...
class Segment():
    ...
    def create_CST_airfoil(self):
        ...
        xx_pre = np.flip(1 - (1 - np.cos(np.linspace(0, 1, int(np.ceil(201/2))) * np.pi)) / 2)              
        xx_suc = (1 - np.cos(np.linspace(0, 1, int(np.ceil(201/2))) * np.pi)) / 2 

        # compute y-coordinates 
        yy_pre = DVGeometryCST.computeCSTCoordinates(xx_pre,Airfoil["N1 lower"],Airfoil["N2 lower"],\
    ...
    def create_PARSEC_airfoil(self):
        ...
        if len(self.LeadingEdgeDevice.PARSEC) != 0:
            ...
            droop_nose_set = [LE_device["delta_s"], LE_device["cs_c"], 
                              ...
                              LE_device["k_Bez1"], LE_device["k_Bez2"]]
            xx_pre,xx_suc,yy_pre,yy_suc = self.deploy_droop_nose(xx_pre,xx_suc,yy_pre,yy_suc,droop_nose_set,w_conic_seal)
